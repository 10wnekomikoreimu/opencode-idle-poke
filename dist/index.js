...
const DEFAULTS = {
    ...
    enabled: true,
    messageTemplate: "[Idle reminder] You have been silent for about {seconds} seconds in session {project} ({sessionID}) - the current active idle-poke interval. " +
    ...
    protocol: "If the user asks to stop the proactive idle-poke, write exactly '{disableMarker}' as the first line of your reply; " +
    ...
    restartNotification: "[System notification] opencode-idle-poke plugin restarted; poke settings have been reset to defaults " +
    ...
    predictor: {
        ...
        model: "",
        timeoutMs: 15_000,
        prompt: "Assess the complexity of the conversation excerpt below and output the reminder-interval marker. " +
...
const DELAY_MARKER_RE = /^[〔\[](?:Remind(?:er)?|提醒间隔)\s*[=＝：:]\s*(\d+(?:\.\d+)?)\s*(s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours|秒|分钟|小时)?[〕\]]$/i;
...
function delayMarkerMs(value, unit) {
    const u = (unit ?? "s").toLowerCase();
    let ms;
    if (u === "秒" || u.startsWith("s"))
        ms = value * 1000;
    else if (u === "分钟" || u.startsWith("m"))
        ms = value * 60_000;
    else if (u === "小时" || u.startsWith("h"))
        ms = value * 3_600_000;
    else
        return undefined;
...
function toConfig(raw) {
    ...
    if (typeof raw !== "object" || raw === null)
        return config;
    const r = raw;
    if (typeof r.idleMs === "number" && Number.isFinite(r.idleMs) && r.idleMs > 0)
        config.idleMs = r.idleMs;
    if (typeof r.requireEngagement === "boolean")
        config.requireEngagement = r.requireEngagement;
    if (typeof r.enabled === "boolean")
        config.enabled = r.enabled;
    if (typeof r.messageTemplate === "string" && r.messageTemplate.trim())
        config.messageTemplate = r.messageTemplate;
    if (typeof r.enableMarker === "string" && r.enableMarker.trim())
        config.enableMarker = r.enableMarker;
    if (typeof r.disableMarker === "string" && r.disableMarker.trim())
        config.disableMarker = r.disableMarker;
    if (typeof r.enablePredictorMarker === "string" && r.enablePredictorMarker.trim())
        config.enablePredictorMarker = r.enablePredictorMarker;
    if (typeof r.disablePredictorMarker === "string" && r.disablePredictorMarker.trim())
        config.disablePredictorMarker = r.disablePredictorMarker;
    if (typeof r.protocol === "string" && r.protocol.trim())
        config.protocol = r.protocol;
    if (typeof r.restartNotification === "string" && r.restartNotification.trim())
        config.restartNotification = r.restartNotification;
    const p = (r.predictor ?? {});
    if (typeof p.enabled === "boolean")
        config.predictor.enabled = p.enabled;
    if (typeof p.maxMessages === "number" && Number.isFinite(p.maxMessages) && p.maxMessages > 0)
        config.predictor.maxMessages = Math.floor(p.maxMessages);
    if (typeof p.agent === "string")
        config.predictor.agent = p.agent;
    if (typeof p.model === "string")
        config.predictor.model = p.model;
    if (typeof p.timeoutMs === "number" && Number.isFinite(p.timeoutMs) && p.timeoutMs > 0)
        config.predictor.timeoutMs = p.timeoutMs;
    ...
    if (typeof l.enabled === "boolean")
        config.logging.enabled = l.enabled;
    if (typeof l.level === "string" && l.level in LOG_LEVELS)
...
async function loadConfig(options) {
    ...
    try {
        ...
        return {
            ...
            error: e instanceof Error && "code" in e && e.code === "ENOENT"
                ? undefined
                : e instanceof Error
                    ? e.message
                    : String(e),
...
export const OpencodeIdlePokePlugin = async (input, options) => {
    const { client } = input;
    ...
    const log = (level, message, extra) => {
        ...
        if (!lg.enabled)
            return;
        if (LOG_LEVELS[level] < LOG_LEVELS[lg.level])
            return;
    ...
    if (loaded.error)
        log("warn", "config load failed, using defaults", { error: loaded.error });
    ...
    const predictorSessions = new Set();
    const buildExcerpt = async (sessionID) => {
        try {
    ...
    const predictInterval = async (sessionID) => {
        const s = sessions.get(sessionID);
        if (!s || !s.predictorEnabled)
            return;
    ...
    return {
        ...
        "chat.message": async (msg, output) => {
            if (predictorSessions.has(msg.sessionID))
                return;
        ...
        "experimental.chat.system.transform": async ({ sessionID }, output) => {
            if (!sessionID)
                return;
            if (predictorSessions.has(sessionID))
                return;
            ensure(sessionID);
            output.system.push(config.protocol
        ...
        "experimental.text.complete": async ({ sessionID }, output) => {
            ...
            for (const line of lines) {
                if (line === config.disableMarker || line === config.enableMarker)
                    continue;
